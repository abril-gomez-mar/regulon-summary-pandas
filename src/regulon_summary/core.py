...
def build_regulon(interactions: pd.DataFrame) -> pd.DataFrame:
    ...
    regulon = (df.groupby('TF').agg(total_genes=("gene", "nunique"), activados=("is_activated", "sum"), reprimidos=("is_repressed", "sum"), genes=("gene", lambda x: ",".join(sorted(set(x))))).reset_index())
